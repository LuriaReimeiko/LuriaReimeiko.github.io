 ...
 */
export function layoutConstellation(seedKey: string, ids: string[], options: LayoutOptions = {}): { nodes: LayoutNode[]; suggestedScale: number } {
    ...
    for (const id of ids) {
        ...
        for (let attempt = 0; attempt < opts.maxAttempts; attempt++) {
            ...
            const minDistToOthers = placed.length === 0 ? Infinity : Math.min(...placed.map((p) => Math.hypot(p.x - x, p.y - y)));
    ...
    const maxReach = placed.length ? Math.max(...placed.map((p) => Math.hypot(p.x - 0.5, p.y - 0.5))) * 2 : 1;
 ...
 */
export function straightPathThrough(points: { x: number; y: number }[], gap: number = 14): string {
